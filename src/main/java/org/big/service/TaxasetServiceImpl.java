...
public class TaxasetServiceImpl implements TaxasetService {
	...
	public JSON findTaxasetList(HttpServletRequest request) {
		...
		for (int i = 0; i < thisList.size(); i++) {
			...
			row.put("tsname", "<a href=\"console/taxaset/show/" + thisList.get(i).getId() + "\" target=\"_blank\">" + thisList.get(i).getTsname() + "</a>");
	...
	@Override
	public JSON findTaxasetListByDatasetId(String dsid, HttpServletRequest request) {
		...
		JSONObject taxasetList = new JSONObject();
 		String searchText = "";
		JSONArray rows = new JSONArray();
		Page<Taxaset> thisPage = this.taxasetRepository.searchByTsname(searchText, dsid, QueryTool.buildPageRequest(offset_serch, limit_serch, sort, order));
		List<Taxaset> thisList = new ArrayList<>();;
		thisList =thisPage.getContent();
		taxasetList.put("total", thisPage.getTotalElements());		// NO1：总数
		for (int i = 0; i < thisList.size(); i++) {
			JSONObject row = new JSONObject();
			Taxaset thisTaxaset = thisList.get(i);
			row.put("id", thisTaxaset.getId());
			row.put("tsname", thisTaxaset.getTsname());
			row.put("tsinfo", thisTaxaset.getTsinfo());
			row.put("bgurl", thisTaxaset.getBgurl());
			rows.add(i, row);
		}
		taxasetList.put("page", offset_serch);						// NO2：offset
		taxasetList.put("rows", rows);								// NO3：Taxon下的所有Dataset
		return taxasetList;
	}

	@Override
	public Boolean countTaxasetsByTsname(HttpServletRequest request) {
		...
		if (StringUtils.isNotBlank(tsname)) {
			num = this.taxasetRepository.countTaxasetsByTsname(tsname, datasetId);
		}
		if (num > 0) {
			return false;
		}else {
