...
public class TraitsetServiceImpl implements TraitsetService {
	...
	public JSON findBySelect(HttpServletRequest request) {
		...
		for (int i = 0; i < thisList.size(); i++) {
			...
			row.put("text", thisList.get(i).getName());
			items.add(row);
		}
	...
	@Override
	public JSON saveOne(@Valid Traitset thisTraitset) {
		JSONObject thisResult = new JSONObject();
		try {
			String traitsetUUID = UUIDUtils.getUUID32();
			thisTraitset.setId(traitsetUUID);
			UserDetail thisUser = (UserDetail) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			thisTraitset.setInputer(thisUser.getId());
			...
			thisResult.put("newId", thisTraitset.getId());
			thisResult.put("newTitle", thisTraitset.getName());
		} catch (Exception e) {
	...
	@Override
	public Boolean countTraitsetsByName(HttpServletRequest request) {
		String name = request.getParameter("name");
		long num = 0;
		if (StringUtils.isNotBlank(name)) {
			num = this.traitsetRepository.countTraitsetsByName(name);
		}
		if (num > 0) {
			return false;
		}else {
