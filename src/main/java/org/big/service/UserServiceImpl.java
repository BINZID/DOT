...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public void addUserBySuper(User thisUser) {
	...
	private void createDefaultTeamAndDatasetForNewUser(User thisUser) {
    	...
    	newTeam.setBgurl("/img/dataset-bg.jpg");
    	newTeam.setStatus("1");
        ...
        newDataset.setBgurl("/img/dataset-bg.jpg");
    ...
    public JSON findbyTeamId(HttpServletRequest request) {
        ...
        for(int i=0;i<thisList.size();i++){
            JSONObject row= new JSONObject();
            String thisEdit1=
                            "<a class=\"wts-table-edit-icon\" onclick=\"transObj('"+teamId+"')\" >" +
                        		"<span class=\"glyphicon glyphicon-transfer\" title=\"权限转让\"></span>" +
                            "</a> &nbsp;&nbsp;&nbsp;" +
            ...
            // -- 团队角色 -- 
            String role="成员";
            Team thisTeam = this.teamRepository.getOne(teamId);
            if(thisTeam.getLeader().equals(thisList.get(i).getId())){
                role="<span class=\"badge bg-light-blue\">负责人</span>";
                row.put("edit",thisEdit1);
            }else {
            	row.put("edit",thisEdit2);
			}
            row.put("role",role);
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            // -- 时间 --
	...
	@Override
	public JSON findUsersByTid(HttpServletRequest request, String tid) {
		String findText = request.getParameter("find");
		...
		JSONArray items = new JSONArray();
		List<User> thisList = new ArrayList<>();
		Team thisTeam = this.teamRepository.findOneById(tid);
		Page<User> thisPage = this.userRepository.searchUsersByTid(tid, thisTeam.getLeader(), findText,
				QueryTool.buildPageRequest(offset_serch, limit_serch, sort, order));
		...
		for (int i = 0; i < thisList.size(); i++) {
			...
			row.put("text", thisList.get(i).getNickname());
			items.add(row);
		}
		thisSelect.put("items", items);
		return thisSelect;
