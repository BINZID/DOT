...
public interface TaxtreeRepository extends BaseRepository<Taxtree, String> {
     ...
     * @return
     */
    @Query(value = "select count(t) from Taxtree t where t.treename = ?1 and t.dataset.id = ?2")
	long countTaxtreesByTreename(String treename, String datasetId);
     ...
     */
    @Query(value = "SELECT "
    		+ "t.id, t.scientificname, t.chname "
    		+ "FROM "
    		+ "Taxon AS t "
    		+ "LEFT JOIN "
    		+ "TaxonHasTaxtree AS tt "
    		+ "ON "
    		+ "t.id = tt.taxonId "
    		+ "WHERE "
    		+ "(t.scientificname like %?1% or t.chname like %?1%) "
    		+ "and "
    		+ "tt.taxtreeId = ?2")
	Page<Object> findTreeNodeByTreeId(String findText, String treeId, Pageable pageable);
