...
public interface TraitsetRepository extends BaseRepository<Traitset, String> {
    ...
    /**
     *<b>根据name统计Traitset实体个数</b>
     *<p> 根据name统计Traitset实体个数</p>
     * @author BINZI
     * @param name
     * @param request 页面请求
     * @return
     */
    @Query(value = "select count(t) from Traitset t where t.name like ?1%")
	long countTraitsetsByName(String name);
    
    /**
	 * <p><b>根据TaxonId查询与之相关的Traitset实体的Id</b></p>
	 * <p>根据TaxonId查询与之相关的Traitset实体的Id</p>
	 * @param id
	 * @return
	 */
/*	@Query(value = "Select t.id from Traitset t Where t.taxon.id = ?1 and t.status = 1")
	List<String> findTraitsetIdListByTaxonId(String id);*/
