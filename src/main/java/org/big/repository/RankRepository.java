...
public interface RankRepository extends BaseRepository<Rank, Integer> {
    ...
    /**
     *<b>根据EnName或chname查找一个实体</b>
     *<p> 据EnName或chname查找一个实体</p>
     * @author BINZI
     * @param name 实体的name
     * @return org.big.entity.Rank
     */
	@Query(value = "Select r from Rank r where r.chname = ?1 or r.enname = ?1")
    Rank findOneByChnameOrEnname(String name);
	/**
     *<b>带分页排序的条件查询</b>
     *<p> 带分页排序的条件查询</p>
     * @author BINZI
     ...
     */
	@Query(value = "Select r from Rank r where (r.enname like ?1% or r.chname like ?1%)")
	Page<Rank> searchByEnname(String findText, Pageable pageable);

	@Query(value = "Select * from rank where enname = ?1", nativeQuery = true)
