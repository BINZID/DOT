...
public interface OccurrenceRepository extends BaseRepository<Occurrence, String> {
	...
	/**
	 * <p><b>根据TaxonId查询与之相关的Occurrence实体的Id</b></p>
	 * <p>根据TaxonId查询与之相关的Occurrence实体的Id</p>
	 * @param id
	 * @return
	 */
	@Query(value = "Select o.id from Occurrence o Where o.taxon.id = ?1 and o.status = 1")
	List<String> findOccurrenceIdListByTaxonId(String id);
	/**
	 * @Description 根据TaxonId删除与之关联的所有观测记录
	 * @author BINZI
	 * @param taxonId
	 */
	@Transactional
    @Modifying
    @Query(value = "delete from occurrence where taxon_id = ?1",nativeQuery = true)
	void delOccurrenceByTaxonId(String taxonId);
}
