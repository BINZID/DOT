...
public interface TeamRepository extends BaseRepository<Team, String> {
	 ...
	 * @return
	 */
	@Query(value ="SELECT t FROM Team AS t LEFT JOIN UserTeam AS ut ON ut.teamId = t.id WHERE ut.userId = ?1 and t.status = 1")	
	List<Team> selectTeamsByUserId(String uid);
    /**
     *<b>根据用户id（分页）查询Team列表（用户是所在Team的Leader）</b>
     *<p> 根据用户id（分页）查询Team列表，即根据用户id查找其所属的Team列表</p>
     * @author WangTianshan (王天山)
     * @param user_id 分页排序方案实体
     * @return org.springframework.data.domain.Page<org.big.entity.Team>
     */			    
	@Query(value ="SELECT t FROM Team AS t LEFT JOIN UserTeam AS ut ON ut.teamId = t.id WHERE ut.userId = ?1 and t.leader = ?1 and t.status = 1")	
    Page<Team> findMyTeamsByUserId(String uid, String searchText, Pageable pageable);

    /**
     *<b>根据用户id（分页）查询Team列表（用户不是所在Team的Leader）</b>
     *<p> 根据用户id（分页）查询Team列表，即根据用户id查找其所属的Team列表</p>
     * @author BINZI
     * @param user_id 分页排序方案实体
     * @return org.springframework.data.domain.Page<org.big.entity.Team>
     */			    
	@Query(value ="SELECT t FROM Team AS t LEFT JOIN UserTeam AS ut ON ut.teamId = t.id WHERE ut.userId = ?1 and t.leader != ?1 and t.status = 1")	
    Page<Team> findJoinTeamsByUserId(String uid, String searchText, Pageable pageable);
     ...
     * @return org.springframework.data.domain.Page<org.big.entity.Team>
     */
    @Query(value = "select t from Team t where (t.name like %?1% or t.leader like %?1%) and t.status = 1")
    ...
    @Query(value = "SELECT " +
            ...
            "AND (" +
            "t.name LIKE %?1% " +
            "OR u.userName LIKE %?1%) AND t.status = 1"
    )
     ...
     * @return void
     */
    @Query("select count(d) from Dataset d where d.team.id = ?1")
    Integer countDatasetsByTeamId(String Id);
    /**
     * <b>通过Id查找一个Team实体</b>
     * <p>	通过Id查找一个Team实体</p>
     * @param teamId
     ...
     * @return
     */
    @Query(value = "select count(t) from Team t left join UserTeam ut on t.leader = ut.userId where t.name = ?1 and t.leader = ?2")
	long countTeamsByName(String name, String uid);
}
