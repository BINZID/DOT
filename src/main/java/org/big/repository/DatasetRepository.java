...
public interface DatasetRepository extends BaseRepository<Dataset, String> {
   ...
   Page<Dataset> searchInfo(String findText, Pageable pageable);
     ...
     * @return org.springframework.data.domain.Page<org.big.entity.Dataset>
     */
    @Query(value = "select d from Dataset d where (d.dsname like %?1% or d.dsabstract like %?1% or d.createdDate like %?1%) and d.team.id = ?2 and d.status = 1")
    ...
    /**
     *<b>根据dsname统计Dataset实体个数</b>
     *<p> 根据dsname统计Dataset实体个数</p>
     * @author BINZI
     * @param dsname
     * @param request 页面请求
     * @return
     */
    @Query(value = "select count(d) from Dataset d where d.dsname = ?1 and d.team.id = ?2")
    long countDatasetsByDsname(String dsname, String teamId);
}
