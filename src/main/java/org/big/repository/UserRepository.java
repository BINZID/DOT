...
public interface UserRepository extends BaseRepository<User, String> {
     ...
     * @return org.springframework.data.domain.Page<org.big.entity.User>
     */
    @Query(value = "select u.nickname, u.email from User u" +
            " where (u.nickname like %?1% or u.email like %?1%) and u.email != ?2")
    Page<Object> findUserInfoByEmail(String findText, String email, Pageable pageable);
	...
	/**
     *<b>Team下的用户select下拉选(分页 查)</b>
     *<p> Team下的用户select下拉选(分页 查)</p>
     * @author BINZI
     * @param request
     * @param pageable
     * @param tid
     ...
     */
	@Query(value = "select u from User u left join UserTeam ut on ut.userId = u.id where ut.teamId = ?1 and ut.userId != ?2")
	Page<User> searchUsersByTid(String tid, String tLeader, String findText, Pageable pageable);
}
